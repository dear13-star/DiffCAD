    ...
    def plot(
        ...
        if pred_boxes is not None and show_boxes:
            for i, d in enumerate(reversed(pred_boxes)):
                ...
                # import cv2
                # img = annotator.result()  # 获取当前图像
                # if c == 1 and text and hasattr(d, 'obb') and d.obb is not None and len(d.obb) >= 4:
                #     w, h = float(d.obb[2]), float(d.obb[3])
                #
                #     if is_obb and isinstance(box, np.ndarray) and box.shape[0] >= 1:
                #         x, y = box[0]
                #     else:
                #         x, y = box[0], box[1]
                #
                #     text_position = (int(x + 10), int(y + 10))
                #     text_position = (
                #         min(text_position[0], img.shape[1] - 120),
                #         min(text_position[1], img.shape[0] - 20)
                #     )
                #
                ...
                #     thickness = 1
                #
                #     # 直接在 annotator 的内部图像上绘制文本
                #     cv2.putText(img, f"W: {w:.2f}, H: {h:.2f}", text_position, font, font_scale, font_color, thickness)
